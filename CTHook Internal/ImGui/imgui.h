...
namespace ImGui
{
	...
	IMGUI_API bool          SliderCheckbox(const char* label, bool* v);
	IMGUI_API int           PiePopupSelectMenu(const ImVec2& center, const char* popup_id, const char** items, int items_count, ImColor col1, ImColor col2, ImColor col3);
	IMGUI_API bool          BeginChildwithTitlebar(const char* str_id, const char* title, const ImVec2& size = ImVec2(0, 0), bool border = false, ImGuiWindowFlags extra_flags = 0);    // req Marc Steiner
	IMGUI_API bool          BeginChildwithTitlebar(ImGuiID id, const char* title, const ImVec2& size = ImVec2(0, 0), bool border = false, ImGuiWindowFlags extra_flags = 0);
